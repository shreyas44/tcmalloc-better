...
fn compile(src_dir: impl AsRef<Path>) {
    ...
    cc.files(FILES.iter().copied().map(join_src_dir));
...
fn patch_deps() -> PathBuf {
    ...
    if out_dir.exists() {
        fs::remove_dir_all(out_dir.as_path()).unwrap();
    }
    fs::create_dir(out_dir.as_path()).unwrap();
    let c_src = PathBuf::from("c_src");
    let c_src_out = out_dir.join(c_src.as_path());
    fs::create_dir(c_src_out.as_path()).unwrap();

    let patch_contents: Vec<_> = fs::read_dir("patches")
        .unwrap()
        .map(|entry| {
            let entry = entry.unwrap();
            let path = entry.path();
            if !entry.file_type().unwrap().is_file() {
                panic!("Patch entry {path:?} is not a file");
            }
            fs::read_to_string(path).unwrap()
        })
        .collect();
    let patches: Vec<_> = patch_contents
        .iter()
        .flat_map(|s| Patch::from_multiple(s).unwrap())
        .collect();
    ...
    while let Some(in_dir) = in_dirs_stack.pop() {
        for in_entry in fs::read_dir(in_dir).unwrap() {
            ...
            let file_type = in_entry.file_type().unwrap();
            let in_path = in_entry.path();
            let out_path = out_dir.join(in_path.as_path());
            if file_type.is_dir() {
